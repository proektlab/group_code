function [X,Y,Z,V]=tubeplot(x,y,z,varargin)  
...
% Written by Anders Sandberg, asa@nada.kth.se, 2005
  ...
  for i=1:N
    X(i,:)=x(i) + r(i,1)*(n(i,1)*cos(theta) + b(i,1)*sin(theta));
    Y(i,:)=y(i) + r(i,2)*(n(i,2)*cos(theta) + b(i,2)*sin(theta));
    Z(i,:)=z(i) + r(i,3)*(n(i,3)*cos(theta) + b(i,3)*sin(theta));
  end
