...
%             [mappedA, network, binary_data, mean_X, var_X] = train_autoencoder(A, net_structure);
        ...
        case {'KPCA', 'KernelPCA'}
			...
			if isempty(varargin), [mappedA, mapping] = kernel_pca_(A, no_dims);
			else [mappedA, mapping] = kernel_pca_(A, no_dims, varargin{:}); end
        ...
        case {'PCA', 'KLM'}
            % Compute PCA mapping
			[mappedA, mapping] = pca_(A, no_dims);
            mapping.name = 'PCA';
            
        case {'SPCA', 'SimplePCA'}
            % Compute PCA mapping using Hebbian learning approach
			[mappedA, mapping] = spca_(A, no_dims);
            mapping.name = 'SPCA';
            
        case {'PPCA', 'ProbPCA', 'EMPCA'}
            % Compute probabilistic PCA mapping using an EM algorithm
			if isempty(varargin), [mappedA, mapping] = em_pca_(A, no_dims, 200);
            else [mappedA, mapping] = em_pca_(A, no_dims, varargin{1}); end
            mapping.name = 'PPCA';
