...
function plotColor3(xValues, yValues, colorIDs, colorMap, lineWidth)
...
for j = 1:length(IDs)
    ...
    for i = 1:length(indices)-1
        ...
        if endIndex ~= 0
            ...
            trace(trace <= 0) = [];
            plot(xValues(trace), yValues(trace), 'LineWidth',lineWidth, 'Color', colorMap(mod(ID-1,size(colorMap,1))+1,:));
