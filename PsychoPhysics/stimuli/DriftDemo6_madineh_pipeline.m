...
%function DriftDemo6_madineh_pipeline(xcenter,ycenter,texsize,contrast_test,contrast_mask)
try
    if exist('params')
    ...
    fr=f*2*pi; %cycles/pixel * 360 degrees/cycle = fr is degrees/pixel
    ...
    % Definition of the drawn source rectangle on the screen:
    src2Rect=[0 0 visiblesize visiblesize];
    ...
    shiftperframe = cyclespersecond * p * waitframes*ifi;
...
while  ~KbCheck
    ...
    while (vbl < vblendtime) && ~KbCheck
        ...
        elseif (0<yoffset-(p-shiftperframe)) && (yoffset-(p-shiftperframe)<shiftperframe)
    ...
    while (vbl < vblendtime) && ~KbCheck
        ...
        dstRect=CenterRectOnPoint(srcRect, xcenter, ycenter);
        Screen('DrawTexture', params.window, gratingtex2, srcRect, dstRect, angle+90, [], [contrast_mask], [], [], [], [0, yoffset, 0, 0]);
    ...
    while (vbl < vblendtime) && ~KbCheck
        ...
        Screen('DrawTexture', params.window, gratingtex2, src2Rect, dstRect, angle+90, [], [contrast_mask], [], [], [], [0, yoffset, 0, 0]);
