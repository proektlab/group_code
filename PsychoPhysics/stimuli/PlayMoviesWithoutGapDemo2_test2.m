function PlayMoviesWithoutGapDemo2_test2(moviename)
...
oldLevel = Screen('Preference', 'VisualDebugLevel', 3); % Disable Psychtoolbox welcome mes
...
try
    ...
    [win]= PsychImaging('OpenWindow',screen,0.5);
    PsychColorCorrection('SetEncodingGamma', win,1/2.08);
    
         
    ix=720;
    iy=480;
    totalframes=1802;
    scale = 2.4; % Don't up- or downscale patch by default.
...
%    %sort according to number
   for i=1:length(moviefiles)
       num(i)=str2num(moviefiles(i).name(11:12));
   end
   [temp,ind]=sort(num(1:end),'ascend');
   
   for i=1:size(moviefiles,1)
       moviefiles2(i).name = [ pwd filesep moviefiles(ind(i)).name ];
   end
    ...
    iteration = 1;
    moviename=moviefiles2(mod(iteration, size(moviefiles,1))).name;
    ...
    while abortit < 2
        ...
        if iteration<40
            ...
            moviename=moviefiles2(mod(iteration, size(moviefiles,1))+1).name;
