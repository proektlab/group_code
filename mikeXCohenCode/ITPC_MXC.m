...
for fi=1:num_frex
    ...
    as = reshape(as,EEG.pnts,EEG.trials); % wavelet decomposition of each time point in each trial - complex numbers 
    
    % compute ITPC
    tf(fi,:) = abs(mean(exp(1i*angle(as)),2)); %absolute value of the mean of the euler's formula of the
